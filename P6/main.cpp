	...
	do {
		...
		case 1:
			...
			do {
				...
				// Sorts Locations
				allLocations.Sort(user.AveragePosition());
		...
		case 3:
			...
			// Check for Covid-19 Exposures
		case 4:

			// Prints out exposure message
			if (user.CheckExposures() == 0) {
				...
				cout << "Found " << user.CheckExposures() << " possible Covid-19 exposures, please arrange to be tested for Covid-19." << endl;
			}
			...
			do {
				...
				case 4:
					...
					if (imperial) {
						...
						inputFloat = getFloatInputRange((float)0.01, (float)9.00);
					}
					// Gets valid metric height
					else {

						inputFloat = getFloatInputRange((float)0.01, (float)2.75);
					}
